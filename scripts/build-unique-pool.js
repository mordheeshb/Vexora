...
async function main() {
  console.log('Testing candidate pool of', candidateIds.length, 'IDs...');
  const validPool = [];
  for (const id of candidateIds) {
    const res = await checkUrl(id);
    if (res.ok && !validPool.includes(id)) {
      validPool.push(id);
    }
  }
  console.log('Found', validPool.length, 'verified unique 200 OK image IDs!');
  
  // Now assign unique URLs to data/products.json, data/categories.json, src/data/vexora-catalog.json
  let poolIdx = 0;
  ...
  // Ensure unique image per product
  ...
  // Ensure unique image per category cover
  ...
  console.log('Updated categories.json with unique images across all subcategories!');
}
